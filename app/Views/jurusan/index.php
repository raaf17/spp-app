...
<?= $this->section('title') ?>
<title>Data Jurusan &mdash; SPPKITA</title>
<?= $this->endSection() ?>
    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="card-header-action mr-1">
          <a href="<?= site_url('jurusan/export'); ?>" class="btn btn-primary"><i class="fa-solid fa-file-export"></i> Export File</a>
          <div class="dropdown">
            <a href="#" data-toggle="dropdown" class="btn btn-primary dropdown-toggle"></i><i class="fa-solid fa-download"></i> Import File</a>
            <div class="dropdown-menu">
              <a href="<?= base_url('jurusan_example_file.xlsx'); ?>" class="dropdown-item has-icon"><i class="fa-solid fa-download"></i> Download Example</a>
              <a href="#" class="dropdown-item has-icon" data-toggle="modal" data-target="#modal-import-jurusan"><i class="fa-solid fa-upload"></i> Upload File</a>
            </div>
          </div>
        </div>
        <div class="card-header-action">
          <a href="<?= site_url('jurusan/trash'); ?>" class="btn btn-danger"><i class="fa fa-trash"></i> Trash</a>
        </div>
      </div>
            ...
            <?php foreach ($jurusan_data as $key => $value) : ?>
              <tr>
                ...
                <td class="text-center" style="width: 15%;">
                  <a href="<?= site_url('jurusan/edit/' . $value->id_jurusan); ?>" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i></a>
                  <form action="<?= site_url('jurusan/delete/' . $value->id_jurusan); ?>" method="post" class="d-inline" id="del-<?= $value->id_jurusan; ?>">
                    <?= csrf_field(); ?>
